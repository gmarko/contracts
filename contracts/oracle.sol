...
contract Oracle is ENSResolvable, Transferrable, Base64, Date, Controllable, ParseIntScientific, TokenWhitelistable {
    ...
    constructor(address _ens_, bytes32 _controllerNode_, bytes32 _tokenWhitelistNode_) public {
    ...
    /// @notice parse cryptocompare data
    /// @param _token ERC20 token address.
    /// @param _result query result in JSON format.
    /// @param _proof origin proof from crypto compare.
    // solium-disable-next-line mixedcase
    function updateTokenRate(address _token, string memory _result, bytes memory _proof) public {
    ...
    */
    ...
    /*
     The following function has been written by Alex Beregszaszi, use it under the terms of the MIT license
    */
    function ecrecovery(bytes32 _hash, bytes memory _sig) private returns (bool _success, address _recoveredAddress) {
        bytes32 r;
        bytes32 s;
        uint8 v;
        if (_sig.length != 65) {
